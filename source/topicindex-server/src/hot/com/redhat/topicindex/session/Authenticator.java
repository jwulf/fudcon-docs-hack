...
public class Authenticator
{
	...
	protected boolean kerberosLogin()
	{
		...
		try
		{
			...
			// loop through the roles in the database and assign them to the
			// user
			/* final EntityManager entityManager = (EntityManager) Component.getInstance("entityManager");
			final List<User> users = entityManager.createQuery(User.SELECT_ALL_QUERY + " where user.userName = 'djorm'").getResultList();
			if (users.size() == 1)
			{
			...
			// This is the FUDCon hack: make all users writers.
	...
	/**
			 * Checks the supplied username and password against a Fedora Account System (FAS) database
			 * 
			 * @return true if the supplied credentials match a FAS user, false
			 *         otherwise
			 */
			protected boolean FASLogin() {
		    	/*
				 	In AS7, add the following XML elements to the standalone.xml file:
				 	
				 	Under the <subsystem xmlns="urn:jboss:domain:security:1.0"> <security-domains> element:
		        ...
		        {
		            System.out.println("Authenticator.FASLogin(): Cannot create LoginContext. " + le.getMessage());
		            return false;
		        } 
		        catch (final SecurityException se) 
		        {
		            System.out.println("Authenticator.FASLogin(): Cannot create LoginContext. " + se.getMessage());
		            return false;
		        } 
		        ...
		        {
		        	// Authentication failed
		        	System.out.println("Authenticator.FASLogin(): Cannot create LoginContext. " + ex.getMessage());
		        	//ex.printStackTrace();
		            return false;
		        }
		    }
			...
			/**
			 * Attempts to login to using the supplied login context and then assigns the roles if successful.
			 * 
			 * @param lc A login context that contains the has already been initialised with the users data.
			 * @throws LoginException If the login fails.
			 */
		    protected void processLogin(LoginContext lc) throws LoginException{
		        lc.login();
		        
		        identity.addRole("basicRole");
				identity.addRole("writerRole");
				
		        // loop through the rolls in the database and assign them to the user
		        /*final EntityManager entityManager = (EntityManager) Component.getInstance("entityManager");
		        ...
		        if (users.size() == 1)
		        {
		        	// keep a list of the roles we have processed, so we don't get caught in a loop
		        	final List<Role> processedRoles = new ArrayList<Role>();
		        	final User user = users.get(0);
		        	for (final UserRole userRole : user.getUserRoles())
		        	{
	...
	/**
	 * Authenticates a user against the Fedora Account System database
	 * @return true if the supplied credentials are valid, false otherwise
	 */
	public boolean FASAuthenticate()
    {
        log.info("authenticating {0}", credentials.getUsername());
  
	    // first attempt a FAS login
	    if (FASLogin())
	    	return true;
