...
public class CustomTagSearch implements DisplayMessageInterface, Serializable
{
	...
	public void buildDocbook()
	{
		...
		try
		{
			...
			if (messageServer != null && messageServerPort != null && messageServerUser != null && messageServerPass != null && messageServerQueue != null)
			{
				...
				buildDocbookMessage.setEntityType(TopicRendererType.TOPIC);
	...
	private void pushToZanataInit() {
		...
		{
			SkynetExceptionUtilities.handleSeamException(ex, false, "Probably an error loading the filter for a zanata push.");
		}
	}
	...
	public void pushToZanataConfirm() {
		...
		for (Topic topic: filterTopics) {
			topics.add(new Pair<Integer, Integer>(topic.getTopicId(), topic.getLatestRevision().intValue()));
			topic.createTranslatedTopic();
		}
