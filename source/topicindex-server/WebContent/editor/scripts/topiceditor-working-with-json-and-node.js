// TODO:
// 1. Switch to Skynet's JSONP interface to allow cross domain scripting and remove the node.js proxy
...
window.timerID=0;
window.clientXSLFile="http://127.0.0.1/docshack/test/xsl/docbook2html.xsl";
...
function doValidate()
{
  ...
  if (divvalidation) {
    divvalidation.innerHTML="Performing validation check...";
    divvalidation.style.visibility="visible";}
  serversideValidateTopic(editor);
}
...
{   
  if ( document.getElementById("button-validate").disabled="false" )
   serversideValidateTopic(editor, "false");
  if (! validXML)
  {
    alert("Please validate XML before saving.");
  }
  else
  { 
    ...
    saveAjaxRequest.onreadystatechange=function()
    {
      if (saveAjaxRequest.readyState==4)
      {
        ...
        {     
          divvalidation.innerHTML="Saved OK";
          divvalidation.style.visibility="visible";
          disableSaveRevert();
        }
        else
        {
          divvalidation.innerHTML="Error saving. Status code: " + saveAjaxRequest.status;
        }
      }
    }
    ...
    saveAjaxRequest.open("POST", restProxy + "restput" + requestString, true);
...
function serversideValidateTopic(editor, verbose){
     ...
     if (ajaxRequest.readyState==4)
     {
        ...
        if (ajaxRequest.status == 200 || ajaxRequest.status == 304)
        {
          ...
          { 
            if (verbose == "true")
            {
              divvalidation.innerHTML="Topic XML is valid Docbook 4.5";
              divvalidation.style.visibility="visible";
            }
            validXML=true;
            ...
            if (verbose=="true")
            {
              divvalidation.innerHTML=ajaxRequest.responseText;
              divvalidation.style.visibility="visible";
            }
            validXML=false;
            return false;
  ...
  ajaxRequest.open("POST", window.previewserverurl + "/topicvalidate", true);
...
function clientsideUpdateXMLPreview(cm, preview){
  ...
  try{  
   ...
   // section=resultDocument.getElementsByClassName("section");
   // if (section !== null)
   // {
      divpreview=document.getElementById("div-preview");
      if (divpreview.hasChildNodes)
...
function serversideUpdateXMLPreview(cm, serverFunction){      
  ...
  if (window.mutex == 0)
  {
    ...
    ajaxRequest.open("POST", window.previewserverurl + "/xmlpreview", true);
    ajaxRequest.setRequestHeader("Content-Type", "text/xml");
...
function loadSkynetTopic(topicID,skynetURL)
{
  ...
  // Load codemirror contents from Skynet URL
    ...
    topicAjaxRequest.open("GET", restProxy + "restget" + requestString, true);
...
function doRevert(){
    loadSkynetTopic(topicID,skynetURL);
}
